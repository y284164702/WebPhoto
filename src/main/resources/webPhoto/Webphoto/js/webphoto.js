...
$(function() {
	...
	$("#number").val(number);
	$.post(ServerURL + "/webPhoto/getPhotos", {token: token}, function(data) {
		if (data.status == 0) {
			photoData = data.data;
			// alert(photoData.length);
			// if(photoData.length > 100){
			// 	number = parseInt(photoData.length / 100) * 100;
			// 	$("#number").val(number);
			// }
			number = $("#number").val();
			// alert(number);
...
var load = function(number) {
	...
	var _out = ['bounceOut', 'bounceOutDown', 'bounceOutLeft', 'bounceOutRight', 'bounceOutUp', 'fadeOut',
		...
		'rotateOut', 'rotateOutDownLeft', 'rotateOutDownRight', 'rotateOutUpLeft', 'rotateOutUpRight', 'slideOutDown',
	...
	var s = setInterval(function() {
		...
		setTimeout(function() {
			...
			// img.setAttribute('src', 'Webphoto/img/b.png');
			img.setAttribute('src', personArray[Math.ceil(Math.random()*personArray.length)].image);
			++CurPersonNum;
			//推送框
			// setTimeout(function() {
	...
	while (m < n) {
		if (photoData.length == 0) {
			for (var i = 0; i < number; i++) {
				personArray.push({
					// image: 'Webphoto/img/a.png'
					image: 'Webphoto/img/wx.jpg'
				});
				m++;
			};
		} else {
			$.each(photoData, function(index, item) {
				// console.log(m);
				if (m >= n) {
					return;
				}
...
layui.use(['form', 'upload'], function() {
	...
	// 	url = url.split("?")[0] +"?number="+data.value;
	// 	// alert(getQueryString("number"));
	...
	var uploadInst = upload.render({
		elem: '#upImg',
		url: ServerURL + '/webPhoto/uploadPhoto', //改成您自己的上传接口
		data: {
			token: token
		},
		auto: true, //自动上传
	...
	$("body>*").bind("click",function(){
		...
		$("img").one("click", function() {
			...
			if (src != 'undefined') {
				...
				if (imgWidth > windowWidth * 0.4) {
					imgWidth = windowWidth * 0.4;
					imgHeight = imgWidth/b;
				}
				if (imgHeight > windowHeight * 0.4) {
					imgHeight = windowHeight * 0.4;
					imgWidth = imgHeight * b;
				}
				// alert("imgWidth="+imgWidth+";imgHeight=;"+imgHeight);

				$("#clickImg").attr("src", src);
				$("#clickImg").attr("width", imgWidth);
				$("#clickImg").attr("height", imgHeight);
				$(".clickImg").show();
				$("#shade").show();
				...
				setTimeout(function() {
					$(".clickImg").hide();
					$("#shade").hide();
					// $("body").css("background", "url(Webphoto/img/bg.jpg)");
				}, 1500);
			}
		});
	});
	...
	$(".clickImg").one("click", function() {
		$(".clickImg").hide();
		$("#shade").hide();
		// $("body").css("background", "url(Webphoto/img/bg.jpg)");
	});
	//减100
	$("#subtraction").one("click", function() {
		var number = getQueryString("number");
		if (number == null) {
			number = $("#number").val()
		};
		if(number == 100){
			...
			var url = window.location.href;
			url = url.split("?")[0] + "?number=" + (Number(number) - 100);
			window.location.href = url;
		}
	});
	//加100
	$("#addition").one("click", function() {
		var number = getQueryString("number");
		if (number == null) {
			number = $("#number").val()
		};
